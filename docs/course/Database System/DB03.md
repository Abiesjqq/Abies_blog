...
SQL conformance levels（标准符合度）分为四级：entry level, transitional, intermediate和full。但没有DBMS达到full SQL level。

SQL分为三部分：

- DDL（data-definition language）：用于创建、删除表、索引、视图、触发器等。
- DML（data-manipulation language）：用于数据的查询、插入、删除、更新操作。
- DCL（data-control language）：用于权限操作。
...
| `numeric(p, d)`             | 定点数类型，总共有 p 位数字，其中小数部分有 d 位，常用于精确计算                         |
...
- `not null`：不允许用null填
- `primary key (K)`：规定主键，默认该属性为not null
...
删除表:`DROP TABLE branch`

修改表：

- 增加列：`ALTER TABLE r ADD column_name`，自动用null填充。
- 删除列：`ALTER TABLE r DROP column_name`。部分数据库中不支持删除列。
- 修改列的类型：`ALTER TABLE r MODIFY (branch_name char(40), assets not null)`，只要类型兼容即可修改。
- 创建index：`CREATE INDEX index_name ON table_name (attribute_list)`。在`INDEX`前可加`UNIQUE`，表示索引列的所有值必须是唯一的。
- 删除index：`DROP INDEX index_name`。
...
- column_list表示返回的列。当连接的表中有同名属性时，必须用`表名.列名`指定。用\*可表示所有列。
- table_list为不同表的笛卡尔积，而不是自然连接。若要表示自然连接，需要在`WHERE`后加上对应属性相等的约束。
- predicate为约束条件，可用and, or, not, between等词表示。

SELECT返回列的特性：

- SELECT对应泛化投影，在`SELECT`后可对列进行运算操作，如加减、数乘。

- SELECT返回结果默认都不去重。在返回的列的前面加`DISTINCT`关键字则表示去重，即返回的整个元组（而不是单个属性值）无重复，e.g.`SELECT DISTINCT branch_name, balance FROM branch`。在列前加ALL表示允许重复，即默认加ALL。

- SELECT后的返回列，可加`AS`表示重命名。FROM后的表名，也可加`AS`重命名，用于WHERE条件中表名的简化。

- 返回值为string时，可用`||`表示返回值的拼接，e.g.`SELECT 'name=' || customer_name FROM customer`，返回显示格式为`name=Abies`。也可用`lower()`和`upper()`转换大小写等。

- 选择时遇到null不忽略。如果选择结果为空，则返回null。

WHERE条件的写法：

- WHERE约束中，可用`BETWEEN AND`表示闭区间。

- `LIKE`表示模糊匹配，其中`%`表示任意多个字符，`_`表示任意单个字符。e.g.`SELECT customer_name FROM customer WHERE customer_name LIKE '%a%'`。需要匹配实际的%和\_时，用`\`转义。

- 判断是否为null时，要用`IS`，而不能用`=`。

`ORDER BY`表示对返回结果排序后输出，默认升序排序。如果排序的列有多列，则优先级依次递减。e.g.` SELECT branch_name FROM loan ORDER BY customer_name, loan DESC`。

### 集合运算

`UNION`表示并，`INTERSECT`表示交，`EXCEPT`表示差。一般用于对SELECT返回结果的操作，UNION表示“或”，INTERSECT表示“与”，EXCEPT表示“但不”。示例：

```sql
(SELECT stu_name FROM student)
UNION
(SELECT stu_name FROM player)
```

集合操作都默认去重。若想保留重复元组，需要在运算后加`ALL`关键字。

### 聚合函数

聚合函数包括`AVG()`, `MIN()`, `MAX()`, `SUM()`和`COUNT()`。使用在SELECT返回结果的定义中，可直接重命名，如`SELECT AVG(balance)avg_bal FROM account`。

如果有GROUP BY，则除了聚合函数中的属性，其他属性只能是GROUP BY的属性，示例：
...
COUNT函数中可加`DISTINCT`，表示相同的记录只计数一次。

COUNT函数忽略null，其他聚合函数都不忽略null。

当需要按聚合结果筛选时，要用`HAVING`关键字，而不能用WHERE（因为WHRER是一行一行判断，而不是整体判断）。示例：
...
一次查询的结果可作为另一次查询的数据表，即nested subqueries（嵌套子查询）。
...
- `SOME`表示存在某些值满足，`ALL`表示所有值都满足。
- `EXISTS`表示检查该tuple是否存在于子查询的数据表中，存在则返回true，否则返回false。`NOT EXISTS`相反。
- `UNIQUE`表示检查该tuple在子查询的数据表中是否唯一，唯一则返回true，否则返回false。`NOT UNIQUE`相反。

自比较时，写成嵌套子查询或自连接。

### 视图

视图（view）即创建虚表，对其进行的所有操作都将转化为对基表的操作，方便使用并保证数据安全。

- 创建视图：`CREATE VIEW view_name AS (SELECT ... FROM ...)`
- 使用视图：直接当成一张表操作
- 删除视图：`DROP VIEW view_name`

如果视图建立在单个表上，且视图的列对应原表的列，则称为**行列视图**。只有行列视图能更新数据。因为插入仍然需要到底层的物理表上执行，复杂视图上插入难编译。
...
**派生表**：嵌套子查询中，可用`AS`将子查询返回内容也重命名为一张表，称为派生表（local view或derived relations）。外层查询时，列名为派生表中的属性、WHERE中用派生表的数据判断。派生表必须重命名（即使没有用到），e.g.：
...
**WITH从句**：通过WITH语句创建的派生表只在当前语句有效，而不是全局有效。

WITH语句创建的派生表定义在选择操作前，SELECT时直接视为数据表使用。e.g.：
...
- 删除元组：`DELETE FROM table_name WHERE ...`
- 插入元组：`INSERT INTO table_name VALUES (attribute_value_list)`。可在表名后加`()`执行插入的列名。
- 更新元组：`UPDATE table_name SET attribute_name = attribute_value WHERE ...`
...
UPDATE时，后一条更新条件可能受前一条更新的影响，要注意先后顺序。可使用case语句：
...
若FROM后直接写多张表，则为多表的笛卡尔积；若使用JOIN，则根据不同条件进行不同连接。

- JOIN condition：表示两表中的哪些tuple参与匹配。
  1. natural：自然连接，共有属性相同时连接。
  2. ON <predicate>：按一定条件连接。只在非自然连接时使用。
  3. USING (...)：括号中列出的属性名相等时连接。只在非自然连接时使用。

- JOIN type：表示未匹配的tuple怎样处理。
...
自然连接和USING条件中，同名属性合并为一列；而用ON表示条件时，同名属性不合并，返回各表中的所有列。
