...
评价索引：访问时间、插入时间、删除时间、空间开销等。

## 顺序索引

顺序文件（sequentially ordered file）：按某个search-key排序存储的数据文件

按索引的search-key和顺序文件的排序键是否相同，分为：

1. **主索引（primary index）**：两者相同，也称为clustering key。主索引下，相邻的key在相邻的磁盘，速度更快。主索引的search-key通常是主键。非序列文件一定没有主索引。
2. **辅助索引（secondary index）**：两者不同，也称为non-clustering key。

辅助索引中，一个搜索键值对应一个索引项，但索引项可能对应多条记录。可以将该索引项指向一个bucket，bucket中保存所有匹配的记录的指针。

按是否每条记录都有索引项，分为：

1. **稠密索引（dense index）**：每条记录都有索引项。辅助索引通常为dense index，因为无法顺序扫描得到相邻数据。查找快，但开销大。
2. **稀疏索引（sparse index）**：只有部分search-key有index entry。通常一个block一个索引项，通过index找到block，再在块中顺序扫描查找。只能是主索引。

**多级索引（multilevel index）**：单层索引太大时，可推广到多层。将primary index本身视为顺序文件，再在上面建立sparse index（一般为B+树）。插入时，要更新所有层的索引。

**_Q：建立索引后，怎么删除记录？_**

1. 根据索引找到记录，从存储中删除。
2. 更新索引文件（考虑单级索引）：
   - dense indices：若search key只对应一条记录，则直接删除；若对应多条，则检查每个指针，如果指向一个记录则删除，指向多记录的第一条则后移一条。
   - sparse indices：若search key没有索引，则不操作；若有，检查下一个search key是否有索引，如果有则不操作，如果没有则用下一个替换。
...
B+树条件, 设 $n$ 是B+树内部节点的最多指针数：

- 从根到叶节点的所有路径等长
- 中间节点（不是根节点、不是叶节点）有 $\lceil n/2\rceil \sim n$ 个孩子
- 叶节点中有 $\lceil (n-1)/2\rceil \sim n-1$ 个值
- 若根节点不是叶子，则至少有2个孩子；若是叶子，则可有 $0\sim n-1$ 个值

对n阶B+树，叶节点中有K个search-key时，树高上限为 $\lceil \log_{\lceil n/2\rceil}K\rceil$。

**插入**：

1. 在文件中写入对应的记录，找到插入的叶节点。
2. 判断search key是否已在叶节点中：
   - 若在：如果需要则新增指针。
   - 若不在：插入叶节点，节点满时分裂并向上传递，直至满足B+树要求。

B+树节点的分裂：

- 叶节点：前 $\lceil n/2 \rceil$ 个值为一组
- 中间节点：前 $\lceil n/2 \rceil -1$ 个值为一组，第 $\lceil n/2 \rceil$ 个值上提
- 根节点：按中间节点分裂，上提的节点成为新根节点

**删除**：

1. 在文件中删除对应的记录，找到叶节点并删除索引。
2. 节点中值不足时，合并节点并向上传递。
3. 如果根节点只有一个孩子，删除根节点。

B+树节点的合并：

- 兄弟节点能补全时：直接合并兄弟节点，并修改上层指针。合并时只需考虑叶节点的左右兄弟，可以用另一子树中兄弟。
- 兄弟节点不能补全时（超出）：重排兄弟节点，并修改上层指针

B+树的空间利用率极大影响收益。为提高空间利用率，在节点分裂和合并时，可让更多兄弟节点参与重新分配。

### LSM树

**_Q：为什么需要LSM tree？_**

B+树插入时可能反复split，每次需要磁盘随机寻道，时间长。LSM树的核心思想为先写内存，满时再批量顺序写入磁盘，从而减少磁盘中random I/O。
