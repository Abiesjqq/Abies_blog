...
总CPU时间为访存指令部分和ALU部分时间相加：

$$
\text{CPU~time}=\text{IC}\times \text{cycle~time}\times \left( \frac{n_{alu}}{n_{inst}}\times T_{alu} + \frac{n_{mem}}{n_{inst}}\times \text{AMAT}\right)
$$
