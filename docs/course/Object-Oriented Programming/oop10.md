...
使用默认参数要主义二义性问题，即不用默认参数的参数类型不能和其他重载函数相同。
...
当且仅当函数参数列表不完全相同时，可以重载。参数不同时，返回类型可以不同。

不能重载的运算符：`.` `.*` `::` `? : ` `sizeof`。不能创造新的运算符、不能改变操作数的个数、不能改变优先级、至少有一个操作数为类对象。
...
对象之间通过消息进行通信。示例为 `objectName.func(param)`，包含接受的对象、执行的方法、参数三部分。

### 成员

Private 指只有当前类能访问，protected 指只有当前类和其派生类能访问。

不声明权限，默认为 private。

### Ctor and Dtor

Ctor 没有返回值，可以有参数；dtor 没有返回值，不能有参数。Dtor 不能重载。

默认构造函数指不带参数的 ctor。编译器自动生成的默认 ctor 不初始化成员变量，若输出则为随机值。

如果自己定义了 ctor，则只能使用手动定义的，不能使用编译器自动生成的默认 ctor。如 `new Point[2]`，对每个 Point 调用默认 ctor。
